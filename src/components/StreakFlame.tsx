...
const StreakFlame: React.FC<StreakFlameProps> = ({ size = 24, className = '', status = 'burning' }) => {
    const isBurning = status === 'burning' || status === 'igniting';
    return (
        <div className={`relative flex items-center justify-center ${className}`} style={{ width: size, height: size }}>

            {/* Unlit State: Wood Logs */}
            {!isBurning && (
                <div className="absolute inset-0 flex items-center justify-center">
                    <svg viewBox="0 0 24 24" fill="none" className="w-full h-full drop-shadow-sm">
                        <path d="M4 18L20 18" stroke="#78350F" strokeWidth="2.5" strokeLinecap="round" /> {/* Bottom Log */}
                        <path d="M6 15L18 15" stroke="#92400E" strokeWidth="2.5" strokeLinecap="round" /> {/* Middle Log */}
                        <path d="M5 18L19 12" stroke="#78350F" strokeWidth="2" strokeLinecap="round" className="opacity-80" /> {/* Cross Log */}
                        <path d="M19 18L5 12" stroke="#78350F" strokeWidth="2" strokeLinecap="round" className="opacity-80" /> {/* Cross Log */}
                    </svg>
                </div>
            )}
