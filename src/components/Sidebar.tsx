...
export const Sidebar: React.FC<SidebarProps> = ({
    ...
    const sidebarVariants = {
        ...
        closed: {
            width: isMobile ? 0 : 72, // 0 width on mobile (hidden), 72px on desktop (collapsed icon view)
            x: isMobile ? -240 : 0,
            transition: { type: "spring", stiffness: 300, damping: 30 }
        }
    } as any;
    ...
    return (
        ...
        `}
            >
                {/* Header / Toggle */}
                <div className={`flex items-center h-14 px-4 ${isOpen ? 'justify-between' : 'justify-center'}`}>
                    {isOpen && (
                        <motion.div
                            initial={{ opacity: 0 }}
                            animate={{ opacity: 1 }}
                        ...
                        >
                            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-gray-700 to-black flex items-center justify-center text-white font-bold text-xs shadow-md border border-white/20 overflow-hidden">
                                {user?.photoURL ? (
                                    <img
                                        src={user.photoURL}
                                        alt="Profile"
